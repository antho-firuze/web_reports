...
class F_Report 
{
	...
	function create($var = NULL, $config = NULL)
	{
		...
		$support = (object)[
			...
			'img_02' 		=> __dir__.'/../images/logo-avrist-asset-management.png',
			'na'	=> 'N/A',
			'breakline1' => '<br></br>',
	...
	function _output($mpdf, $config)
	{
		if ($config->output) {
			...
			if (defined('FCPATH'))
				return [TRUE, ['link' => $config->urlPath.$filename, 'path' => $config->absPath.$filename]];	
			else 
				exit(json_encode(['link' => $config->urlPath.$filename, 'path' => $config->absPath.$filename]));
		}
		$mpdf->Output();
	}
}
